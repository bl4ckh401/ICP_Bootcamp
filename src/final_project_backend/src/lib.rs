...
thread_local! {
    ...
    static PROPOSAL_MAP: RefCell<StableBTreeMap<u64, Proposal, Memory>> = RefCell::new(StableBTreeMap::init (MEMORY_MANAGER.with(|n| n.borrow().get(MemoryId::new(0)))));
}
    ...
    PROPOSAL_MAP.with(|p| 
        {
            ...
            match choice{
                Choice::Approve => proposal.approved += 1,
                Choice::Pass => proposal.passed -= 0,
                Choice::Reject => proposal.rejected -= 1,
            }
